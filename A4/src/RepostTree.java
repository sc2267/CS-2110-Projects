/* NetId(s): sc2267. Time spent: 5 hours, 10 minutes.
...
public class RepostTree {
    ...
    public RepostTree insert(Person c, Person p) throws IllegalArgumentException {
        //TODO 1. This function has a simple, non-recursive implementation
           if (c==null || p==null) 
        	   throw new IllegalArgumentException("Either c or p is null");
           if (getTree(p)==null || getTree(c)!= null) 
        	   throw new IllegalArgumentException("P is null or C already exists");
           
            RepostTree pRoot = getTree(p);		
			RepostTree cChild = new RepostTree(c);
			pRoot.children.add(cChild);
			return pRoot;
    ...
    public int size() {
        //TODO 2
           if (getChildrenCount()==0) 
           		return 1;
           int sum = 1;
           for (RepostTree people : children) {        	
           		sum = sum + people.size();
            }
    		return sum;
       }
    ...
    public int depth(Person p) {
    	...
    	int depthnum = 0; 
   		for (RepostTree person : children) {
   		       depthnum = depthnum + person.depth(p) +1;
   			} 
   			return depthnum; 
   		}
    ...
    public int widthAtDepth(int d) throws IllegalArgumentException {
        ...
        //            of the children at depth d-1.
    	 if (d<0) {
                 throw new IllegalArgumentException("d is less than 0");
    	 }
    	 else if (d==0) {
                 return 1;
         } 
         int width= 0; 
    	 for (RepostTree person : children) {
         		width = width + person.widthAtDepth(d-1);
         	}
    	 return width; 
         }
    ...
    public List<Person> getRepostRoute(Person c) {
        ...
        if (getTree(c)==null) {
			...
			List<Person> RepostRoute= new LinkedList<Person>();
            RepostRoute.add(c);
            return RepostRoute;
       }
       
        for (RepostTree person : children) { 
    	  		if (person.getRepostRoute(c) !=null) {
    	  			List<Person> Second = person.getRepostRoute(c);
    	  			Second.add(0, root);
    	  			return Second;
    	  		}
    	  	}
    ...
    public Person getSharedAncestor(Person child1, Person child2) {
        ...
        if (getTree(child1)== null || getTree(child2) == null) {
       		return null;
         }
        
       List <Person> Ancestor1 = getRepostRoute(child1);
       List <Person> Ancestor2 = getRepostRoute(child2);
       Person one[] = Ancestor1.toArray(new Person[Ancestor1.size()]);
       Person two[] = Ancestor2.toArray(new Person[Ancestor2.size()]);
       
       Person sharedAncestor = null;
       int a = 0;
       int b = 0;
       while (a< one.length && b< two.length && 
       		one[a].equals(two[b])) {
       		sharedAncestor = one[a];
       		a++;
       		b++;
       }
    ...
    public boolean equals(Object ob) {
    	...
    	if (ob instanceof RepostTree == false) { 
    		return false;
    	} 
    	RepostTree A = (RepostTree) ob;
    	if (A.getRoot().equals(root) == false) {
    		return false;
    	} 
   
    	if (A.getChildrenCount() != getChildrenCount()) {
    		return false;
    	} 
    
    	for (RepostTree person : children) {
    		
    		if (sortingOut(person, A.getChildren()) != true) {
    			return false;
    		} 
         }   
    	return true;
    ...
    private boolean sortingOut(RepostTree B, Set<RepostTree> C) {
    		for (RepostTree person : C) {
    				if (B.equals(person)) {
    					return true;
    				}
    		}
