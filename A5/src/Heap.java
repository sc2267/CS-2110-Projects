/** NetId(s): sc2267. Time spent: 8 hours, 30 minutes.
 *
 ...
 * Initially, It took me a while to understand this assingment and its methods because of the complexity
 * of the heaps. But I reviewed the lecture notes thoroughly and some of the given methods in the notes
 * were very helpful in writing the given methods we had to do. 
 */
...
public class Heap<E> {
    ...
    protected void ensureSpace() {
    	...
    	if (size==d.length) { 
    	    VP[] dcopy = Arrays.copyOf(d, size*2); 
    	    d=dcopy; 
    	} 
    ...
    void swap(int h, int k) {
        ...
        VP[] exchange= createVPArray(1);
        exchange[0]= d[h]; 
        map.put(exchange[0].val, k); 
        d[h]=d[k]; 
        map.put(d[k].val, h); 
        d[k]=exchange[0]; 
        }
    ...
    void bubbleDown(int k) {
        ...
        assert 0 <= k  &&  k < size;
        int e= upperChild(k);
         while ( e< this.size && compareTo(e,k)>0) {
              swap(k, e);
              k= e;
              e= upperChild(k); 
          }
     ...
     *      index of the one that must appear above the other in a heap. */
       protected int upperChild(int n) {
    	   ...
    	   else if (d[a].priority==d[c].priority) { 
    		   return c; 
    	   } 
    	   else if  (compareTo(a,c)>0) { 
    		      return a; 
    		   } 
    		   else { 
    		      return c; 
    ...
    public E poll() {
        ...
        //         two values with the same priority are not swapped.
    	 if (size == 0) {
    		 throw new NoSuchElementException("No Elements can be found in the heap");
    	 }
    	 E v=  d[0].val;   
    	 swap(0,size-1); 
    	 map.remove(d[size-1].val,size-1); 
    	 size= size-1; 
    	 if (size>0) { 
    	 bubbleDown(0);  
    	 } 
